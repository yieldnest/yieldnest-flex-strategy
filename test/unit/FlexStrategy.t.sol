    ...
    function testFuzz_processAccounting_preventsRewardsExceedingTargetApy(uint128 deposit) public {
        ...
        /**
            ...
            3.  the Variable is called targetAPY but that's not what it is. it is actually APR.
                So assuming all the other issues are perfectly fixed, setting it to 15% (APR) means you're capping at 16%+ APY per year.

            4.  Assuming all other issues addressed, The approach can still be operationally difficult.
                Assume that you have  a per day targetAPY value. if rewards are not processed for multiple days in a row,
                You've essentially missed slots, and you potentially can't reach targetAPY because it does not factor 
